...
user()
{
  mkdir -p ${uzip}/usr/home/solobsd/Desktop
  # chroot ${uzip} echo furybsd | chroot ${uzip} pw mod user root -h 0
  chroot ${uzip} pw useradd solobsd -u 1000 \
  -c "SoloBSD User" -d "/home/solobsd" \
  -g wheel -G operator -m -s /usr/local/bin/mksh -k /usr/share/skel -w none
  chroot ${uzip} pw groupadd solobsd -g 1000
  # chroot ${uzip} echo furybsd | chroot ${uzip} pw mod user liveuser -h 0
  chroot ${uzip} chown -R 1000:1000 /usr/home/solobsd
  chroot ${uzip} pw groupmod wheel -m solobsd
  chroot ${uzip} pw groupmod video -m solobsd
  chroot ${uzip} pw groupmod webcamd -m solobsd
}
